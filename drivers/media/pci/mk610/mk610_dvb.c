...
struct sec_priv {
	struct mk610_adapter *adap;
	int (*set_voltage)(struct dvb_frontend *fe,
			   enum fe_sec_voltage voltage);
};

static int mk610_set_voltage(struct dvb_frontend* fe,
		enum fe_sec_voltage voltage)
{
	struct sec_priv *priv = fe->sec_priv;
	struct mk610_gpio_config *cfg = &priv->adap->cfg->gpio;
	struct mk610_dev *dev = priv->adap->dev;
	...
	switch (voltage) {
		case SEC_VOLTAGE_13:
			mk610_gpio_set_pin(dev, &cfg->lnb_power, 1);
			mk610_gpio_set_pin(dev, &cfg->lnb_voltage, 0);
			break;
		case SEC_VOLTAGE_18:
			mk610_gpio_set_pin(dev, &cfg->lnb_power, 1);
			mk610_gpio_set_pin(dev, &cfg->lnb_voltage, 1);
			break;
		default: /* OFF */
			//mk610_gpio_set_pin(dev, &cfg->lnb_power, 0);
			break;
	}
...
static void mk610_release_sec(struct dvb_frontend* fe)
{
...
static struct dvb_frontend *mk610_attach_sec(struct mk610_adapter *adap, struct dvb_frontend *fe)
{
	struct sec_priv *priv;
	...
	fe->ops.set_voltage = mk610_set_voltage;
	fe->sec_priv = priv;

	return fe;
}

static int set_mac_address(struct mk610_adapter *adap) 
{
...
static int start_feed(struct dvb_demux_feed *dvbdmxfeed)
{
	...
	struct mk610_adapter *adapter = dvbdmx->priv;
...
{
	struct mk610_i2c *i2c_adap = i2c_get_adapdata(i2c);
	struct mk610_dev *dev = i2c_adap->dev;
	...
	case SEC_VOLTAGE_13:
		...
		break;
	case SEC_VOLTAGE_18:
		GPIO_VALUE |= MK610_GPIO_PIN(rf_in, 0);
		GPIO_VALUE |= MK610_GPIO_PIN(rf_in, 1);
		break;
	...
	pci_write(MK610_GPIO_BASE, 0, GPIO_VALUE);
	return 0;
}
...
static int mk610_frontend_attach(struct mk610_adapter *adapter)
{
	struct mk610_dev *dev = adapter->dev;
	...
	//	adapter->fe->ops.diseqc_send_burst = max_send_burst;
		...
		if (mk610_attach_sec(adapter, adapter->fe) == NULL) {
			dev_warn(&dev->pci_dev->dev,
	...
	strlcpy(adapter->fe->ops.info.name,mk610_boards[pci->subsystem_vendor].name,52);
	return 0;
...
frontend_atach_fail:
	...
	adapter->fe = NULL;
	dev_err(&dev->pci_dev->dev, "mk610 frontend %d attach failed\n",
		adapter->nr);

	return -ENODEV;
}

int mk610_dvb_init(struct mk610_adapter *adapter)
{
	struct mk610_dev *dev = adapter->dev;
	...
	ret = dvb_register_adapter(adap, "mk610 DVB Adapter",
					THIS_MODULE,
...
void mk610_dvb_exit(struct mk610_adapter *adapter)
{
	struct dvb_adapter *adap = &adapter->dvb_adapter;
	struct dvb_demux *dvbdemux = &adapter->demux;

	if (adapter->fe) {
		// mk610_ca_release(adapter);
